            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach($paginatedRecipes as $recipe)
                    <a href="{{ route('recipes.show', $recipe->id) }}"
                       class="bg-white shadow-md rounded-lg p-4 text-center transition duration-300 transform hover:scale-105 hover:shadow-xl block group">
                        <img src="{{ $recipe->thumbnail_url }}" alt="{{ $recipe->name }}"
                             class="w-full h-48 object-cover mb-4 rounded group-hover:opacity-90">
                        ...
                        <div class="flex flex-col justify-between items-center">
                            ...
                            @elseif(count($recipe->missing_ingredients) > 0)
                                <span class="bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full"
                                      title="Missing: {{ implode(', ', $recipe->missing_ingredients) }}">
                                {{ count($recipe->missing_ingredients) }} Missing Ingredient{{ count($recipe->missing_ingredients) > 1 ? 's' : '' }}
                            </span>
                            @endif
                            ...
                            <div class="flex justify-between items-center w-full mt-2">
                                ...
                                <form method="POST" action="{{ route('recipes.bookmark', $recipe->id) }}">
                                    @csrf
                                    <button
                                        class="{{ in_array($recipe->id, $savedRecipeIds) ? 'text-red-500 hover:text-gray-500' : 'text-gray-400 hover:text-red-500' }} transition duration-300"
                                        aria-label="Bookmark">
